...
public class BaseController {
	...
	public @ResponseBody List<String> getStreamingTweets(@PathVariable String keywords, @RequestParam(required = false) boolean storeData, 
		...
		BlockingQueue<String> queue = new LinkedBlockingQueue<String>(TWEET_LIMIT);
		TwitterStatusListenerImpl twitterStatusListener = new TwitterStatusListenerImpl(db2JsonStore, collectionName, storeData, queue);
		...
		final List<String> collected = new ArrayList<String>(TWEET_LIMIT);
        while (collected.size() < TWEET_LIMIT) {
            // TODO: Handle InterruptedException
            String tweet = null;
			try {
				tweet = queue.poll(10, TimeUnit.SECONDS);
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				log.error("Queuing error");
			} 
	...
	public @ResponseBody List<String> getTimelineTweets(@PathVariable String screenName, @RequestParam(required = false) boolean storeData, 
			@RequestParam(required = false) String collection) throws TwitterException {
	...
	public @ResponseBody List<String> getDBTweetsBasedOnKeyword(@RequestParam(required = true) String query,
			@RequestParam(required = false) String collection) throws JsonParseException, JsonMappingException, IOException {
		...
		if (dbHandle != null) {
			...
			} else if (dbCursorForKeyword != null) {
			...
			} else if (dbCursorForEntireCollection != null) {
