...
<body
	...
	<script>
		...
		$(function() {
			$('#storeDBCheckBox').click(
					function() {
						var satisfied = $('#storeDBCheckBox:checked').val();
						if (satisfied != undefined) {
							$('#db2PropertiesFileTextbox').removeAttr(
									'disabled');
							$('#collectionTextBox').removeAttr('disabled');
						}

						else {
							$('#db2PropertiesFileTextbox').attr('disabled',
									'disabled');
							$('#collectionTextBox')
	...
	<script>
		...
		function setDB() {
			...
			if (propertiesFileName.length > 0) {
				$
						.ajax({
							url : 'dbStore?propertiesFileName='
									+ propertiesFileName,
							type : "POST",
							...
							error : function(e) {
								$('#dbInfo')
										.html(
												'Sorry.. Couldn\'t set your DB.. Please check the configuration again.');
							}
						});
			}
		}
		...
		function doGoAction() {
			...
			var storeData = false;
			if($('#storeDBCheckBox').is(':checked')) {
				storeData = true;
			}
			//document.write(storeData);
			if (storeData) {
				$('#dbInfo').html("Data to be stored");
				setDB();
				/* $
						.ajax({
							url : 'dbStore?propertiesFileName='
			...
			$
					.ajax({
						url : tweetType + '/' + keyword + '?storeData='
						...
						success : function(data) {
							...
							$('#resultTextArea').text(tweets);
							//$('#dbInfo').html("Tweets being retrieved..");
						},
						error : function(e) {
							$('#dbInfo')
									.html(
											'Oops!! Something went wrong while getting the Tweets..');
						}
					});
		...
		function doSubmitAction() {
			var keyword = $('#queryTextBox').val();
			var collection = $('#collectionTextBox').val();
			setDB();
			$.ajax({
				url : "retrieveTweets?query=" + keyword + "&collection="
						+ collection,
				//dataType : "json",
				type : "GET",
				success : function(data) {
					...
					$('#resultTextArea').text(tweets);
					//$('#dbInfo').html('Retrieving Tweets from DB');
				},
				error : function(e) {
					$('#dbInfo').html('');
				}
	...
	<div class="container" id="Json">
		...
		<div>
			<table
				style="cell-padding: 3px; cell-spacing: 2px; margin-left: 375px;">
				<tr>
					<td colspan="2">
				...
				<tr>
					<td colspan="2" align="center"><input type="submit" value="GO"
						...
						class="btn btn-large" onclick="doGoAction()" /></td>
				</tr>
			</table>
		</div>
		<!-- 		</fieldset>
 ...
 -->
		...
		<div>
			<table
				style="cell-padding: 3px; cell-spacing: 2px; width: 800px; height: 650px; margin-left: 175px">
				<tr>
					<td>
				...
				<tr>
					<td><textarea id="resultTextArea" name="resultText"
							readonly="readonly"
							style="width: 800px; height: 400px; max-height: 400px; background-color: #E6E6FA; align: center">							
						 </textarea></td>
				</tr>
				<tr>
					<td>
				...
				<tr>
					<td align="center"><a type="submit" id="querySubmit"
						...
						class="btn btn-large" onclick="doSubmitAction()">Submit</a></td>
				</tr>
			</table>
		</div>
		<br /> <br />
